...
Часть A перенесена: ветка `claude/cell-as-task-layer-kbepuw` в `wforkorg/notch`
(`task_io.py`, `board.py`, тесты на их `conftest.py`, `pydantic>=2`). Здесь лежит та же
версия файлов плюс копия `hub_io.py`, чтобы тесты шли без второго репозитория.
Пункт 1 ниже закрыт по `gated_bridge.py`; пункт 2 — `occurred_at` у proposal нет, берётся
`created_at`/`updated_at`, если хаб их отдаёт.

## Что должна была выяснить сессия с доступом к коду (не оператор)
